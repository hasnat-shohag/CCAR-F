...
keywords:
  - term: Task tool
    bn: Coordinator যে টুল দিয়ে subagent spawn করে (বর্তমান Claude Code-এ নাম Agent; Task alias)।
    tip: allowedTools-এ Task/Agent না থাকলে spawn অসম্ভব — binary gate।
  - term: allowedTools
    bn: Coordinator কোন টুল চালাতে পারবে তার তালিকা; subagent spawn-এর অনুমতি এখানেই।
    tip: Agent বাদ পড়লে unattended run-এ permission denial-এ সব spawn আটকে যায়।
  - term: AgentDefinition
    bn: Subagent-এর সংজ্ঞা — description, system prompt ও tool restrictions।
    tip: Description-ই coordinator-কে বলে কখন কোন subagent ডাকবে।
  - term: structured metadata
    bn: সোর্স-তথ্য (source_url, document_name, page_number) কনটেন্ট থেকে আলাদা রেখে পাঠানোর কাঠামো।
    tip: Metadata বাদ দিলে synthesis এজেন্ট citation দিতে পারে না।
  - term: parallel spawning
    bn: এক response-এ একাধিক Task tool call — স্বাধীন subagent একসাথে চালানো।
    tip: '"single response" বা "simultaneously" মানেই parallel উত্তর।'
  - term: fork_session
    bn: শেয়ার্ড বেসলাইন থেকে স্বাধীন শাখা তৈরি (SDK-তে forkSession)।
    tip: fork = branch, resume = append — দুটো আলাদা কাজ।
  - term: --resume
    bn: আগের নামযুক্ত সেশন চালিয়ে যাওয়ার কন্ট্রোল।
    tip: SDK-তে resume-এর পাশে fork_session দিলে append বদলে branch হয়।
sources:
...
পরীক্ষার গাইড v1.0 `--resume` আর `fork_session`-কে দুটি session control হিসেবে দেখায়, পরীক্ষায়ও সেই কাঠামোই। Agent SDK-তে (sessions guide, সেপ্টেম্বর ২০২৬ মিলিয়ে দেখা) fork হল resume-এর উপর বসানো modifier, বিকল্প নয়। দুটোই একসাথে দিতে হয়: `ClaudeAgentOptions(resume=session_id, fork_session=True)`। `resume` বলে কোন সেশন থেকে শুরু, আর `fork_session` বলে সেটির উপর append না করে শাখা বানাও। `fork_session` না দিলে একই call আসল সেশনে যোগ হয়। CLI-তেও জোড়া একই: `--fork-session` কেবল `--resume` বা `--continue`-এর همراهে কাজ করে। তাই পরীক্ষা যাচাই করে append বনাম branch — দুটো আলাদা কমান্ড নয়। পরীক্ষায় গাইডের ভাষাতেই উত্তর দিন: continue করতে resume, branch করতে fork।

...
এজেন্টিক লুপ হলো Claude-ভিত্তিক প্রতিটি এজেন্টের মূল এক্সিকিউশন সাইকেল। এটি প্রম্পটের কৌশল নয়, retry loop নয়, চ্যাটবটের টার্নও নয় — এটি কোডে লেখা ডিটারমিনিস্টিক কন্ট্রোল ফ্লো। এই লাইফসাইকেল ঠিকভাবে বুঝলে Domain 1-এর বড় অংশ আপনাআপনি বসে যায়; ভুল বুঝলে প্রোডাকশনে এজেন্ট কাজের মাঝপথে থেমে যাবে।

## এজেন্টিক লুপের লাইফসাইকেল

কাজ শেষ না হওয়া পর্যন্ত চারটি ধাপ ঘুরতে থাকে:

1. **Messages API-তে রিকোয়েস্ট পাঠান।** এর সাথে conversation history যাবে — system prompt, আগের মেসেজ, আর আগের ইটারেশনের টুল ফল।
2. **রেসপন্সের `stop_reason` ফিল্ড পড়ুন।** এই ফিল্ডটাই ঠিক করে দেয় পরের ধাপ কী। এজেন্টিক লুপে প্রাসঙ্গিক মান দুটি:
   - `"tool_use"` — Claude এক বা একাধিক tool call করতে চায়। লুপ চলবে।
   - `"end_turn"` — Claude কাজ শেষ করেছে। লুপ থামবে।
3. **`stop_reason` যদি `"tool_use"` হয়:** টুল চালান, ফলটি নতুন মেসেজ হিসেবে conversation history-তে যোগ করুন, তারপর হালনাগাদ history নিয়ে Claude-এর কাছে আবার পাঠান।
4. **`stop_reason` যদি `"end_turn"` হয়:** এজেন্ট শেষ। চূড়ান্ত উত্তর ব্যবহারকারীকে দেখান।

```mermaid
flowchart TD
  A["Messages API-তে রিকোয়েস্ট<br/>system prompt + history + টুল ফল"] --> B["response.stop_reason পড়ুন"]
  B -->|tool_use| C["টুল চালান"]
  C --> D["tool_result মেসেজ<br/>history-তে যোগ করুন"]
  D --> A
...
টুল ফল অবশ্যই conversation history-তে যোগ করতে হবে। এটা বাদ দিলে Claude পরের ইটারেশনে নতুন তথ্য নিয়ে ভাবতেই পারবে না — টুল কী ফেরত দিল, সেটাই সে দেখে না, তাই নতুন কিছু নিয়ে কাজ করার মতো কিছু তার হাতে থাকে না।

## মূল ধারণা

`stop_reason` ফিল্ডই লুপ কন্ট্রোলের একমাত্র নির্ভরযোগ্য সংকেত। এটি ডিটারমিনিস্টিক ও দ্ব্যর্থতাহীন। প্রাকৃতিক ভাষার লেখা parse করা, টেক্সট কনটেন্ট দেখে সিদ্ধান্ত নেওয়া, বা ইচ্ছেমতো iteration cap বসানো — কোনোটিকেই লুপ থামানোর মূল প্রক্রিয়া বানাবেন না।
...
- `model_context_window_exceeded` — রেসপন্স মডেলের context window ভরিয়ে দিয়েছে; `max_tokens` ট্রাঙ্কেশনের মতোই ধরুন।
...
এজেন্টিক লুপে Claude বর্তমান কনটেক্সট দেখে ঠিক করে কোন tool call করবে — এটাই model-driven decision-making। মডেল কাজটা পড়ে, হাতের টুলগুলো ভাবে, তারপর একটা বাছে। এর বিপরীতে আছে আগেই ঠিক করা decision tree বা fixed tool sequence, যেখানে ডেভেলপার হার্ডকোড করে দেন কখন কী চলবে।

পরীক্ষায় model-driven পদ্ধতিকে প্রাধান্য দেওয়া হয়, কারণ এটা নমনীয়। ডেভেলপার যেসব পরিস্থিতি আগে থেকে ভাবেননি, মডেল সেগুলোও সামলায়, edge case handle করে, এমন ক্রমে টুল চালায় যা কেউ পরিকল্পনা করেনি। তবে একটি ব্যতিক্রম মুখস্থ রাখুন: ব্যবসার যুক্তি ডিটারমিনিস্টিক কমপ্লায়েন্স দাবি করলে — আর্থিক লেনদেন, নিরাপত্তা যাচাই, নিয়ন্ত্রক শর্ত — তখন প্রোগ্রাম্যাটিক এনফোর্সমেন্ট এই নমনীয়তার চেয়ে উপরে। বিস্তারিত Task Statement 1.4-এ।
...
### অ্যান্টি-প্যাটার্ন ২: ইচ্ছেমতো iteration cap-কে মূল স্টপিং সিস্টেম বানানো
...
`response.content[0].type == "text"` দেখে লুপ শেষ ধরে নেওয়া। ভুল, কারণ Claude `tool_use` ব্লকের পাশাপাশি টেক্সটও ফেরত দিতে পারে। একটা রেসপন্সে ব্যাখ্যামূলক লেখা ("I'll now search for the customer's order history") থাকতেই পারে, ঠিক তার পরপরই tool call। টেক্সট আছে কি না, তা দিয়ে এজেন্ট শেষ হয়েছে কি না জানা যায় না।
...
<ExamTrap>
	<p>
		<strong>ইচ্ছেমতো iteration cap-কে (যেমন "১০ লুপ পরেই থামাও") মূল স্টপিং সিস্টেম বানানো</strong> — ক্যাপ হয় দরকারি কাজ কেটে দেয়, নয় অপ্রয়োজনীয় ইটারেশন চালায়। safety net ঠিক আছে, মূল কন্ট্রোল নয়।
	</p>
</ExamTrap>
...
> একজন ডেভেলপারের এজেন্ট মাঝে মাঝে অকালে থেমে যায়, যখন Claude tool call-ের সাথে টেক্সটও ফেরত দেয়। লুপটি `response.content[0].type == 'text'` দেখে ধরে নেয় এজেন্ট শেষ। ব্যবহারকারীরা জটিল প্রশ্নে অসম্পূর্ণ উত্তর পাচ্ছেন। ডেভেলপারের কী বদলানো উচিত?
...
<BuildExercise
	...
	objectives={[
		...
		'বহু-টার্ন এক্সিকিউশনের জন্য টুল ফল history-তে যোগ করা',
	...
	steps={[
		{
			...
			why: 'বহু-টুল সেটআপ model-driven সিদ্ধান্ত দেখায় — কনটেক্সট পড়ে Claude ঠিক টুলটি বাছতে হয়, যা এজেন্টিক আর্কিটেকচারের মূল কথা।',
			expect:
		...
		{
			title:
				'tool_use পেলে টুল চালান, tool result মেসেজ বানান, আর তা conversation history-তে যোগ করুন।',
			why: 'এটাই লুপের সবচেয়ে গুরুত্বপূর্ণ হ্যান্ডঅফ — পরীক্ষা দেখে আপনি ঠিকভাবে tool call বের করেন কি না এবং সঠিক মেসেজ ফরম্যাটে ফল ফেরত দেন কি না।',
			expect:
				'Claude টুল চাইলে কোডটি tool_use ব্লক বের করে, সংশ্লিষ্ট ফাংশন চালায়, এবং assistant রেসপন্সসহ tool_result-যুক্ত user মেসেজ history-তে যোগ করে।',
		},
		{
		...
		{
			title:
				'এমন প্রম্পট দিয়ে টেস্ট করুন যাতে পরপর কয়েকটি tool call লাগে (যেমন: আগে একটি মান খুঁজুন, তারপর তা দিয়ে হিসাব করুন) এবং সব ইটারেশনে লুপ ঠিকভাবে চলছে কি না দেখুন।',
			why: 'ক্রমিক tool call পুরো লাইফসাইকেল যাচাই করে — একটি টুল শেষ করে ফল পেয়ে সিদ্ধান্ত নিয়ে পরের টুলে যেতে হয়।',
			expect:
				'end_turn-এর আগে অন্তত দুটি tool call ইটারেশন। এজেন্ট আগে সার্চ করবে, ফল হিসাবে ব্যবহার করবে, শেষে সম্মিলিত উত্তর দেবে।',
		},
		{
			title:

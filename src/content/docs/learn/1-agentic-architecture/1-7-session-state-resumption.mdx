...
flowchart TD
  ...
  R -->|হ্যাঁ, ফাইল অপরিবর্তিত| A["--resume<br/>history চালু রাখুন"]
...
**কখন ব্যবহার করবেন না:** শেষ সেশনের পর ফাইল বদলেছে। history-র tool result আর codebase-এর বর্তমান অবস্থা মেলে না। ফলে নিচের stale context সমস্যা দেখা দেয়।
...
SDK-তে এটি resume-এর বদলে নয়, পাশে সেট করতে হয়: `resume` সেশন বাছে, আর `fork_session: true` সেই history-র কপি থেকে নতুন সেশন শুরু করে, যোগ করে না। CLI-তে `--fork-session` একইভাবে `--resume`-এর সাথে জোড়া লাগে। (Agent SDK sessions guide, সেপ্টেম্বর ২০২৬-এ মিলিয়ে দেখা; বিস্তারিত নোট Lesson 1.3-এ।)
...
**কখন ব্যবহার করবেন:** আগের সেশনের tool result বাসি (ফাইল বদলেছে, API আপডেট হয়েছে, dependency সরে গেছে)। দীর্ঘ সেশনে কনটেক্সট ডিগ্রেড হয়েছে (প্রয়োজনহীন tool result-এ history ভরে গেছে)। জ্ঞান ধরে রেখে পরিষ্কার বেসলাইন দরকার।
...
**কীভাবে ফুটে ওঠে:** এক ডেভেলপার Claude Code দিয়ে codebase বিশ্লেষণ করেন। এরপর ৩টি ফাইল বদলে সেশন resume করেন। Claude ওই ৩ ফাইল নিয়ে পরস্পরবিরোধী পরামর্শ দেন — এমন পরিবর্তনের সুপারিশ করেন যা ইতিমধ্যে হয়ে গেছে, বা এমন কোডের কথা বলেন যা আর নেই — কারণ তিনি history-তে থাকা পুরনো tool ফল দেখে ভাবছেন।
...
**সহজ fix যা যথেষ্ট নয়:** সেশন resume করে এজেন্টকে শুধু বদলানো ফাইলগুলো আবার পড়তে বলা। কিছু না করার চেয়ে ভালো, কিন্তু বাসি tool result history-তে রয়ে যায়। মডেল তখনো কনটেক্সটের পুরনো তথ্য ধরে যুক্তি করতে পারে, বিশেষত এমন সিদ্ধান্তে যা বদলানো ফাইলের সাথে সরাসরি জড়িত নয়।
...
<ExamTrap>
	<p>
		<strong>ফাইল বদলের পর resume করার পর শুধু "আবার পড়ে নাও" বলা</strong> — পরীক্ষায় এটাই সবচেয়ে ভালো উত্তর নয়। বাসি ফল history-তে থেকে যুক্তিকে প্রভাবিত করে; সামারি-ইনজেক্ট করা নতুন শুরু বেশি নির্ভরযোগ্য।
	</p>
</ExamTrap>
...
| --- | --- | --- |
| গতকালের কাজ চালু রাখা, ফাইল বদলায়নি | `--resume` | আগের কনটেক্সট বৈধ, পুরো history কাজে লাগে |
...
| দীর্ঘ সেশন, history ভরা | নতুন শুরু + সামারি | ডিগ্রেড কনটেক্সটে পরিষ্কার বেসলাইন ভালো |
...
<ExamTrap>
	<p>
		<strong>ফাইল বদলের পর <code>--resume</code> সুপারিশ করা</strong> — resume বাসি tool result history-তে রেখে দেয়; এজেন্ট পুরনো কোড দেখে ভাবতে পারে। সামারি-ইনজেক্ট করা নতুন শুরু এই ফাঁদ এড়ায়।
	</p>
</ExamTrap>
...
<details>
	...
	**কেন বাকিগুলো ভুল:**
	...
	- *সেশন resume রেখে শুধু বদলানো ফাইল আবার পড়তে বলা* — বাসি tool result history-তে থেকে যায়; মডেল তবুও পুরনো তথ্য ধরে যুক্তি করতে পারে। এটাই ক্লাসিক ভুল।
...
<BuildExercise
	...
	steps={[
		...
		},
		{
			...
			expect: 'history-র বাসি tool result থেকে যাওয়া তথ্য উল্লেখ করে এজেন্ট দ্বিধান্বিত বা অসামঞ্জস্যপূর্ণ উত্তর দেয়।',
		},
		{
			title:
			...
			expect: 'নতুন সেশনে আগের history নেই, তবু সামারি থেকে সমস্যার প্রেক্ষাপট পুরোপুরি স্পষ্ট।',
		},
		{

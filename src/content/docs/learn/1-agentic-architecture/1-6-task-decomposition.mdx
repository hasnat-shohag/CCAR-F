...
keywords:
  - term: fixed sequential pipeline
    bn: আগেই ঠিক করা ধাপের ক্রম (prompt chaining); প্রতি ধাপ আগের output ইনপুট নেয়।
    tip: পূর্বানুমেয়, স্ট্রাকচার্ড কাজে ভালো — অপ্রত্যাশিত ফল সামলাতে পারে না।
  - term: dynamic adaptive decomposition
    bn: প্রতিটি ধাপে যা মেলে তার ভিত্তিতে পরের উপকাজ তৈরি — পরিকল্পনা চলতে চলতে বদলায়।
    tip: Open-ended investigation-এ (legacy codebase, security audit) দরকার।
  - term: attention dilution
    bn: এক pass-এ অনেক আইটেম দিলে মনোযোগ ভাগ হয়ে গভীরতা অসমান হয়।
    tip: শুরুর ফাইলে গভীর বিশ্লেষণ, শেষের ফাইলে সামান্য — এটাই লক্ষণ।
  - term: multi-pass architecture
    bn: প্রতি আইটেমে আলাদা local pass + শেষে একটি cross-item integration pass।
    tip: বড় মডেল বা বড় কনটেক্সট উইন্ডো এই সমস্যার সমাধান নয়।
  - term: cross-item integration pass
    bn: সব local pass শেষে চলা pass, যা আইটেম-ছাড়ানো বিষয় (data flow, pattern consistency) খোঁজে।
    tip: Batch ভাগ করলে cross-batch সমস্যা থেকে যায় — এই pass সেটি ধরে।
sources:
...
এটাই attention dilution। সমাধান আরও শক্তিশালী মডেল নয়, বড় কনটেক্সট উইন্ডো নয়, আরও বিস্তারিত prompt নয়। সমাধান কাঠামোগত: ১৪টি আলাদা per-file analysis pass (প্রতিটি একটি ফাইলে মনোযোগী) এবং একটি cross-file integration pass (সব ফাইলে data flow ও pattern consistency যাচাই)।
...
<ExamTrap>
	<p>
		<strong>Attention dilution-এর সমাধান হিসেবে আরও শক্তিশালী মডেল বা বড় কনটেক্সট উইন্ডো</strong> — এটি আর্কিটেকচারাল সমস্যা, মডেল-সামর্থ্যের নয়। এক pass-এ বেশি আইটেম দিলে গভীরতা অসমান হবে, মডেল যতই শক্তিশালী হোক।
	</p>
</ExamTrap>
...
<details>
	...
	**কেন বাকিগুলো ভুল:**
	...
	- *বড় কনটেক্সট উইন্ডোর মডেলে আপগ্রেড* — মনোযোগ-বণ্টনের সমস্যা কনটেক্সট সাইজ দিয়ে মেটে না।

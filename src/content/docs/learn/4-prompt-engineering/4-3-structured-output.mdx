...
keywords:
  - term: tool_use with JSON schemas
    bn: টুলের JSON schema দিয়ে Claude-এর ফেরত ডেটার গঠন বেঁধে দেওয়া।
    tip: JSON syntax error (bracket, trailing comma) একেবারে দূর করে; কিন্তু ভুল মান আটকায় না।
  - term: prompt-based JSON
    bn: টেক্সট উত্তরের ভেতরে JSON লিখতে বলা, কোনো schema-বাধা ছাড়া।
    tip: Production-এ মাঝেমধ্যে unparseable আউটপুট দেয়; নির্ভরতার দিক থেকে নিচু স্তর।
  - term: '"auto"'
    bn: মডেল নিজে ঠিক করে টুল কল করবে কি না; টেক্সটেও উত্তর দিতে পারে।
    tip: "Auto মানে structured output-এর কোনো নিশ্চয়তা নেই।"
  - term: '"any"'
    bn: মডেলকে টুল কল করতেই হবে, তবে কোন টুল তা মডেল বাছে।
    tip: ডকুমেন্টের ধরন অজানা কিন্তু structured output নিশ্চিত চাইলে এটিই।
  - term: forced selection
    bn: "`{type: tool, name: ...}` দিয়ে নির্দিষ্ট টুল কল করতে বাধ্য করা, যেমন extract_metadata।"
    tip: Tool_choice প্রতি request-এ প্রযোজ্য; পরের request-এ না সরালে একই টুল আবার কল হবে।
  - term: semantic error
    bn: গঠন ঠিক কিন্তু মান ভুল — যোগফলের গরমিল, ভুল field-এ মান, অনুপস্থিত তথ্যের বদলে বানানো মান।
    tip: Schema কেবল গঠনের জামিন; correctness আলাদা ভ্যালিডেশন দাবি করে।
  - term: nullable/optional field
    bn: উৎস ডকুমেন্টে তথ্য না থাকলে field-টি null ফেরাতে দেওয়ার সুযোগ।
    tip: Fabrication ঠেকানোর প্রধান উপায় — required field মডেলকে মান বানাতে চাপ দেয়।
  - term: "unclear enum value"
    bn: উৎস সত্যিই অস্পষ্ট হলে শ্রেণিবিন্যাস না করে "unclear" ফেরানোর অনুমতি।
    tip: জোর করে শ্রেণিবিন্যাস ঠেকায়; অন্য দিকে "other" + detail string নতুন ক্যাটাগরি ধরে।
sources:
...
পরীক্ষা এই ক্রমের উপরেই দাঁড়ায়। Tool use-এ টুলের JSON schema Claude-এর ফেরত ডেটার আকার বেঁধে দেয়; বন্ধনী হারানো, trailing comma বা unquoted key-র মতো syntax সমস্যা আর হয় না। আর `tool_choice` প্যারামিটারটিই মডেলকে টুল কল করতে বাধ্য করে। Prompt-based extraction (টেক্সট উত্তরে JSON লিখতে বলা) কোনো গঠন-নিশ্চয়তা দেয় না; production-এ মাঝেমধ্যেই unparseable আউটপুট দেবে।

## tool_choice-এর তিন মোড

`tool_choice` প্যারামিটার ঠিক করে মডেল টুল কল করবে কি না, আর কীভাবে করবে। পরীক্ষার জন্য তিনটি মোড চিনে রাখুন:

- **`"auto"` (ডিফল্ট):** মডেল নিজে সিদ্ধান্ত নেয় টুল কল করবে, নাকি টেক্সটে উত্তর দেবে। Extraction টুলের বদলে সে conversationally টেক্সটও ফেরাতে পারে। মডেলের সত্যিই কথোপকথনের স্বাধীনতা দরকার হলে এটি ব্যবহার করুন।
- **`"any"`:** মডেলকে টুল কল করতেই হবে, তবে কোন টুল তা সে বাছে। একাধিক extraction schema থাকলে (যেমন `extract_invoice`, `extract_receipt`, `extract_contract`) আর ডকুমেন্টের ধরন অজানা হলে এটি ব্যবহার করুন। নিশ্চিত structured output, টুল বাছার স্বাধীনতাও আছে।
- **`{"type": "tool", "name": "extract_metadata"}`:** মডেলকে হুবহু নির্দিষ্ট টুল কল করতে বাধ্য করে। বাধ্যতামূলক প্রথম ধাপ নিশ্চিত করতে ব্যবহার করুন — যেমন enrichment ধাপের আগে metadata extraction চালানো।
...
`extract_metadata` এখানে আপনার নিজের বানানো টুল; নাম যা খুশি হতে পারে। `tool_choice` প্রতি request-এ কাজ করে, পুরো conversation জুড়ে নয়। বাধ্য করা কলটি ফেরার পর পরের request-এ `auto` দিন (বা প্যারামিটার বাদ দিন), নইলে মডেল আবার একই টুল কল করতে বাধ্য হবে — লুপ।
...
<ExamTrap>
	<p>
		<strong>tool_choice <code>"auto"</code>-কে <code>"any"</code> ভেবে ফেলা</strong> — <code>auto</code> টুল না কল করে টেক্সট ফেরানোর স্বাধীনতা দেয়, তাই structured output-এর নিশ্চয়তা নেই। <code>any</code> টুল কল নিশ্চিত করে, তবে কোন টুল তা মডেল বাছে। ডকুমেন্টের ধরন অজানা কিন্তু নিশ্চিত structured output চাইলে <code>any</code>।
	</p>
</ExamTrap>
...
<BuildExercise
	...
	steps={[
		...
		{
			title: 'tool_choice any-তে বদলে যাচাই করুন মডেল সর্বদা টুল কলের মাধ্যমে structured output দেয়।',
			why: 'tool_choice any টুল কল নিশ্চিত করে, আর কোন টুল তা মডেলকে বাছতে দেয়। ডকুমেন্টের ধরন অজানা কিন্তু structured output নিশ্চিত চাইলে এটিই সঠিক সেটিং।',
			expect: 'প্রতিটি রেসপন্সে stop_reason = tool_use, এবং schema-মেনে বৈধ structured output-সহ টুল কল। কেবল টেক্সটে কোনো উত্তর নেই।',
		},
		{

...
MCP resource tool call ছাড়াই এজেন্টের সামনে কনটেন্ট ক্যাটালগ আনে। কোন ডেটা আছে তা জানতে exploratory টুল না ডেকে, এজেন্ট তথ্যটি আগেই পায়।
...
লাভ হলো কম নষ্ট কল। Resource ছাড়া এজেন্ট হয়তো `list_tables` ডাকে, তারপর প্রতিটি table-এ `describe_table` — শুধু দিক ঠিক করার জন্য tool call পুড়িয়ে ফেলে। Database schema resource থাকলে সে সাথে সাথেই জানে।
...
> একটি টিম Claude Code ওয়ার্কফ্লোতে issue tracking-এর জন্য Jira ইন্টিগ্রেট করতে চায়। একজন ডেভেলপার custom MCP server বানানোর প্রস্তাব দিলেন। সঠিক প্রথম ধাপ কোনটি?

<details>
	<summary>উত্তর দেখুন</summary>

	**সঠিক উত্তর:** Jira-র জন্য বিদ্যমান community MCP server আছে কি না দেখা, আর সেগুলো team-specific চাহিদা মেটাতে না পারলেই কেবল custom বানানো।

	**কেন বাকিগুলো ভুল:**

	- *Jira ইন্টিগ্রেশন ~/.claude.json-এ রাখা* — এটি user-level ও ব্যক্তিগত, version control-এ থাকে না। টিমের সবার দরকারি server-এর জায়গা `.mcp.json`।
	- *MCP বাদ দিয়ে Bash থেকে সরাসরি Jira REST API ডাকা* — server setup বাঁচে ঠিকই, কিন্তু টুল ডিসকভারি, স্কোপিং আর টিম-শেয়ারিং সব হাতছাড়া হয়।
	- *টিমের দরকারি endpoint ধরে custom MCP server বানানো* — পরিস্থিতিতে team-specific চাহিদা স্পষ্ট না থাকলে custom build ভুল উত্তর। Standard ইন্টিগ্রেশনে community server-ই প্রথম পছন্দ।
...
<BuildExercise
	...
	objectives={[
		...
		'MCP resource দিয়ে exploratory tool call কমানো',
	...
	steps={[
		...
		},
		{
			...
			why: 'Resource ছাড়া এজেন্ট list_tables তারপর প্রতিটি টেবিলে describe_table ডেকে tool call নষ্ট করে।',
			expect:

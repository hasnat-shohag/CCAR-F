...
**মূল ধারণা:** Scoping প্যাটার্ন Claude Code জুড়ে এক — **project-level (`.claude/`) git-এ শেয়ার হয়; user-level (`~/.claude/`) ব্যক্তিগত।** এটি CLAUDE.md, commands/skills আর rules-এ খাটে। Domain 3 জুড়ে এই প্যাটার্ন মুখস্থ করুন। দুটো project-scoped path-ই একই command বানায়; `.claude/skills/` canonical ও বেশি-feature-যুক্ত। তবে ফাইল-আকার মিলিয়ে রাখুন: skill directory, ভেতরে `SKILL.md`; command flat `.md`।
...
> একটি টিম চায় `/review` কমান্ডটি রিপো clone করা সবার পাওয়া উচিত। একই ডেভেলপার চান একটি ব্যক্তিগত `/brainstorm` skill, যেটি মূল কথোপকথন না ভরিয়ে verbose codebase বিশ্লেষণ দেবে। কোনটি কোথায় বানাতে হবে, আর skill-টির কী কনফিগারেশন লাগবে?

<details>
	<summary>উত্তর দেখুন</summary>

	**সঠিক উত্তর:** `/review` `.claude/commands/`-এ (টিম-শেয়ারিং), আর `/brainstorm` `~/.claude/skills/brainstorm/SKILL.md`-এ `context: fork` frontmatter-সহ।

	**কেন বাকিগুলো ভুল:**

	- *দুটোই `~/.claude/commands/`-এ, README-তে কপি করার নির্দেশ দিয়ে* — User-level কমান্ড সবার কাছে পৌঁছায় না, আর হাতে কপি করা মানে drift।
	- *দুটোই `.claude/commands/`-এ, brainstorm-এ `context: fork`* — তাহলে ব্যক্তিগত skill টিমের সবার কাছে চলে যায়, অথচ চাহিদা ছিল ব্যক্তিগত।
	- *`/review` CLAUDE.md-এ, `/brainstorm` `.claude/skills/`-এ* — CLAUDE.md always-on নিয়মের জায়গা, ডাকার মতো কমান্ডের নয়; আর টিম-শেয়ার্ড `/review`-এর ঘর `.claude/commands/`।
